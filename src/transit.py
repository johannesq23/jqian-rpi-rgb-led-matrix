import requests
from google.transit import gtfs_realtime_pb2
from google.protobuf.json_format import MessageToDict
import time

url = "https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/nyct%2Fgtfs-bdfm"

# # headers = {"x-api-key": MTA_API_KEY}
# resp = requests.get(url)

# feed = gtfs_realtime_pb2.FeedMessage()
# feed.ParseFromString(resp.content)

# feed_dict = MessageToDict(feed)

# entries =  feed_dict["entity"]

# for entry in entries:
#   route_id = entry.get("tripUpdate", {}).get("trip", {}).get("routeId")
#   if route_id == "F":
#     print(entry)


# Fetch the feed
resp = requests.get(url)

# Parse protobuf
feed = gtfs_realtime_pb2.FeedMessage()
feed.ParseFromString(resp.content)

# Convert to dict for easier access (optional)
feed_dict = MessageToDict(feed)

# Target stop ID (example: 34th Street–Herald Sq F train)
target_stop = "F14N"

# Collect departures
departures = []

for entity in feed_dict.get("entity", []):
  trip_update = entity.get("tripUpdate", {})
  if not trip_update:
    continue
  
  trip_id = trip_update.get("trip", {}).get("tripId", "unknown")
  route_id = trip_update.get("trip", {}).get("routeId")
  
  for stu in trip_update.get("stopTimeUpdate", []):
    stop_id = stu.get("stopId")
    if stop_id == target_stop:
      # arrival or departure times (epoch seconds)
      arrival = stu.get("arrival", {}).get("time")
      departure = stu.get("departure", {}).get("time")
      
      # convert to human-readable
      if departure:
        departure_time = time.strftime('%H:%M:%S', time.localtime(int(departure)))
      else:
        departure_time = None
        
      departures.append({
        "trip_id": trip_id,
        "route": route_id,
        "departure_time": departure_time
      })

# Show departures
for d in departures:
  print(d)